    ...
    window.SBD = {
        render: function(data, c) {
            ...
            function renderData(data) {
                ...
                renderIdealLine(d3, svg, chart.line, data.start, data.plannedPoints, data.end);
            ...
            function renderAxis(d3, svg, timeDomain, burndowns) {
                ...
                x.domain(timeDomain);
                y.domain([0, d3.max(burndowns, function(d) { return d.points; })]);
            ...
            function renderIdealLine(d3, svg, line, startDate, plannedPoints, endDate) {
                var ideal = [{date: startDate, points: plannedPoints}, {date: endDate, points: 0}];
            ...
            function renderComments(d3, svg, chart, data) {
                ...
                group.selectAll("path")
                    .data(data.burndowns)
                    .enter().append("path")
                        .attr("transform", function(d) { return "translate(" + chart.x(d.date) + "," + chart.y(d.points) + ")"; })
                        .attr("d", d3.svg.symbol())
                        .on("mouseover", function(d) {
                            var x = chart.x(d.date) + 20,
                                y = chart.y(d.points) - 50,
                                margin = 10,
                                width = 200,
                                height = 100;
                            ...
                            div.append("hr");
                            div.append("div").html('<div class="comment">Remaining: ' + d.points + "</div>");

                            if (d.comment) {
                                div.append("div").html('<div class="comment">Story Cards: ' + d.comment + "</div>");
                            }
                        })
